...
res_time_keys=list(df['UploadResponseTime'].index)

# print(res_time_keys)

res_time_vals = df['UploadResponseTime'].values
...
df["CPUServiceTime"]=(df["user+system(%)_vmstat"]*int(ncores)*1000)/(df["UploadThroughput"]*100)


df["CalcThinkTime"]=((df["# users"]*1000)/df["UploadThroughput"])-df["UploadResponseTime"]
...
"""
axs.plot(df["# users"],df["UploadThroughput"],marker='.')
axs.set_xlabel("# users")
axs.set_xticks(np.arange(df["# users"].min(),df["# users"].max(),x_axis_tick_size))
axs.set_xticklabels(axs.get_xticks(), rotation = 45)
axs.set_ylabel("Upload Throughput(req/sec)")
y_axis_tick_size= round((df["UploadThroughput"].max()-df["UploadThroughput"].min())/len(df.index),2)
axs.set_yticks(np.arange(df["UploadThroughput"].min(),df["UploadThroughput"].max(),y_axis_tick_size))
...
axs.set_xticklabels(axs.get_xticks(), rotation = 45)
"""
axs.plot(df["# users"],df["UploadResponseTime"],marker='.')
axs.set_xlabel("# users")
axs.set_xticks(np.arange(df["# users"].min(),df["# users"].max(),x_axis_tick_size))
axs.set_ylabel("Upload Response time(msec)")
y_axis_tick_size= round((df["UploadResponseTime"].max()-df["UploadResponseTime"].min())/len(df.index),2)
axs.set_yticks(np.arange(df["UploadResponseTime"].min(),df["UploadResponseTime"].max(),y_axis_tick_size))
axs.set_xticklabels(axs.get_xticks(), rotation = 45)
